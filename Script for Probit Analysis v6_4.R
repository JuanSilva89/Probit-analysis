...
# R version 4.2.2 (2023-02-14)


# UPDATES:
# Probit analysis is aborted control mortality is higher than 10%.

# Probit analysis is aborted when any dose has a percent mortality equal or lower than controls or 100% mortality.

# This script includes LC1 and LC99 in the output matrix.
...
## create two data frames (one with the controls only and another with rest of the data)
...
abbot.mort<-round((dataf.output$mort-datac$mort)/(1-datac$mort),digits=3)  ###### abbot.mort should replace the previous mortality from the optimization
  

probmort<-sapply(abbot.mort,qnorm) # apply probit transformation to the probability of mortality
...
total.tested<-paste("Total number of individuals tested =",sum(N)+datac$total) # Total number of individuals tested includes controls and doses used to create the probit line
...
if(min(datac$mort*100>10)){warn.control.mort<-c("WARNING: CONTROL MORTALITY EXCEEDING 10%")}
if(min(datac$mort*100<10)){warn.control.mort<-c(" ")}
warn.control.mort
...
# Abbott's correction to include in the output file
raw.abbott.correction<-round((dataf.output$mort-datac$mort)/(1-datac$mort)*100,digits=3) 


# Warning message: No controls added
if (min(dataf$dose)==0) {control.mort<-c("*** PLEASE ADD CONTROLS ***")}
if (min(dataf$dose)>0) {control.mort<-matrix(0,nrow=1,ncol=2)
...
if (min(dataf.output$mort) <= min(datac$mort)) {
  output<-c(" ")
...
if(length(strains)==1) {
  ...
  write("Script version: v6.4", exporttab, append=T)
