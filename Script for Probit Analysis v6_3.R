...
if (cont==1) {  # Apply Abbott's correction 
  ...
  for (i in 1:length(strains)) { # the procedure is done for each strain in turn 
    ii<-datafc$strain == strains[i]
...
if(length(strains)==1) {
  ...
  write("Script version: v6.3", exporttab, append=T)
