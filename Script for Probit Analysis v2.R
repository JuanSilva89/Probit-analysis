...
if (min(datac$dead)>0.00) {cont<-1}  # if mortality in the controls is always below 5%, there is no need for correction
if (cont==1) {  #if mortality in the controls is over 5%, we use the Abbott's correction 
...
output<-cbind(dataf$dose,round(dataf$mort*100,digits = 3),"",round(dataf$probit,digits = 3),dataf$total,dataf.output$dead,round(new.expected.mortality,digits = 3),"",round(chitt.output,digits = 3),extreme.values)
